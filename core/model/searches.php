...
class Searches_Model
{
	...
	{
		$this->table_name = 'searches'; // nazwa głównej tabeli modelu w bazie
	}
	...
	public function SetPages($value, $limit, $show_rows)
	{
		...
		$filter = empty($value) ? NULL : " AND (search_text LIKE '%" . $value . "%' OR agent LIKE '%" . $value . "%' OR user_ip LIKE '%" . $value . "%')";
	...
	public function GetAll($limit, $params)
	{
		...
		$filter = empty($_SESSION['list_filter']) ? NULL : " AND (search_text LIKE '%" . $_SESSION['list_filter'] . "%' OR agent LIKE '%" . $_SESSION['list_filter'] . "%' OR user_ip LIKE '%" . $_SESSION['list_filter'] . "%')";

		$this->rows_list = array();

		$query = 	"SELECT id, agent, user_ip, search_text, search_time" .
