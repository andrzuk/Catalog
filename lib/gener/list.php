...
class ListBuilder
{
	...
	public function build_list()
	{
		...
		if (!empty($_SESSION['list_filter'])) $this->title .= ' z: {' . $_SESSION['list_filter'] . '}';
		...
		$main_text .= '<table class="Table" width="100%" cellpadding="2" cellspacing="3">';
		...
		if (is_array($this->list))
		{
			foreach ($this->list as $item => $row)
			{
				...
				{
					// kolumna akcji:
					$main_text .= '<td class="DataCell" style="text-align: ' . $this->attribs[$idx]['align'] . ';">';
